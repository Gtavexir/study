...
class GConGreedy:
    ...
    def __init__(self, data, initial_adj=None, occl=False, metric='c', limit = 2.0, ls =False):
        self.occl = occl
        self.ls = ls
    ...
    def construct(self):
        ...
        for it in range(10000):
            num_succ = 0
            for q, qvec in enumerate(tqdm(self.vectors, desc=f"grd-{it}-{self.limit}-build")):
            ...
            if it % 10 == 0:
                self.save_adj_as_file(f"adj_iter_{self.limit}_{self.prefix}_{it}")

            num_succ_real = 0
            for q, qvec in enumerate(tqdm(self.vectors, desc=f"grd-{self.limit}-{it}-test")):
...
parser.add_argument('-l', metavar='EDGE_GENERATION_LIMIT', help='edge generation limit')
...
gcg = GConGreedy(data, initial_graph, occl=args.occl, metric =args.m, limit=args.l, ls = args.ls)
