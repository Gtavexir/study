...
class GConGreedy:
    ...
    def construct(self):
        ...
        for it in range(10000):
            num_succ = 0
            for q, qvec in enumerate(tqdm(self.vectors, desc=f"grd-{it}-{self.percent}-build")):
            ...
            if it % 10 == 0:
                self.save_adj_as_file(f"adj_iter_{self.percent}_{self.previous}_{it}")

            num_succ_real = 0
            for q, qvec in enumerate(tqdm(self.vectors, desc=f"grd-{self.percent}-{it}-test")):
